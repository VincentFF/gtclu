...
class GridTree:
    ...
    def build_tree(self, which_level, grids, dim_bounds):
        ...
        if l_edges and r_edges:
            ...
            for i, indexs in enumerate(l_indexs):
                if indexs.any():
                    l_edges[i].extra_weight = 0.5 * r_edges_avg_weight * len(indexs)
                    l_neighbors[l_edges[i].pos] = [r_edges[j] for j in indexs]
            for i, indexs in enumerate(r_indexs):
                if indexs.any():
                    r_edges[i].extra_weight = 0.5 * l_edges_avg_weight * len(indexs)
    ...
    def _cluster_leaf_node(self, node):
        ...
        ball_tree = BallTree(centers, leaf_size=50, metric="euclidean")
        ...
        for i, index in enumerate(indexs):
            ...
            grids[i].extra_weight = (len(index) - 1) * avg_weight * 0.5

            if grids[i].weight + self.min_pts >= self.min_pts:
